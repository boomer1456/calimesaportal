...
async function restart() {
  ...
  // PGLite loads pglite.data / pglite.wasm from the same directory as the
  // bundled module. Nitro does not copy those sidecars, so local `vite preview`
  // (no DATABASE_URL) crashes on first getSql(). Deployed Vercel uses Neon.
  ...
  if (existsSync(pgliteDest) && existsSync(pgliteSrc)) {
    for (const file of ["pglite.data", "pglite.wasm", "initdb.wasm"]) {
      const from = join(pgliteSrc, file);
      if (existsSync(from)) copyFileSync(from, join(pgliteDest, file));
    }
  }
