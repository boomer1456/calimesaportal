...
const PANE_COPY = {
  ...
  forms: {
    kicker: "House binder",
    title: "Forms",
    blurb: "Print, download, or fill on this phone. Saved p-cards sit in Archived receipts.",
  },
  radio: {
    kicker: "RRU V24A6",
    title: "Radio",
    blurb: "Live lookup from the 2024 RRU / CAL FIRE load. Confirm with the engine radio.",
  },
} as const;
...
export function Policy() {
  ...
  useBackLayer(archiveOpen, () => setArchiveOpen(false));
  useBackLayer(Boolean(openDoc), () => setOpenDoc(null));
  useBackLayer(Boolean(fillId), () => setFillId(null));

  const copy = PANE_COPY[pane];

  if (fillId === "P-Card" || fillId?.startsWith("P-Card:")) {
    const receiptId = fillId.startsWith("P-Card:") ? fillId.slice("P-Card:".length) : undefined;
    return <ProcurementForm onBack={() => go()} receiptId={receiptId} />;
  }
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="block text-xs text-cream/70">
                Snap a receipt — the form fills in
              </span>
            </span>
          </button>
          <button
            ...
            className="flex min-h-11 items-center gap-3 rounded-md border border-line bg-surface-2 px-3 py-2.5 text-left"
          >
            <span className="flex size-10 shrink-0 items-center justify-center rounded-sm bg-navy text-cream">
              <FolderOpen className="size-5" />
            </span>
            <span className="min-w-0 flex-1">
              <span className="block text-sm font-semibold text-ink">Archived receipts</span>
              <span className="block text-xs text-muted">Saved p-card tickets — tap one to open</span>
            </span>
          </button>
          <a
  ...
  return (
    <section className="flex flex-col gap-4 pb-6">
      <button
        type="button"
        onClick={onBack}
        className="inline-flex h-11 items-center gap-2 self-start text-sm font-medium text-navy"
      >
        <ArrowLeft className="size-4" /> Back
      </button>
      <header>
        <p className="text-xs font-medium tracking-[0.18em] text-muted uppercase">Forms</p>
        <h1 className="font-display text-4xl font-bold leading-none tracking-tight text-navy">
          Archived receipts
        </h1>
        <p className="mt-2 text-sm leading-relaxed text-muted">
          Tap a ticket to edit it. Delete removes it from the house archive.
        </p>
      </header>
      <label className="flex h-11 items-center gap-2 rounded-sm border border-line bg-surface-2 px-3">
        <Search className="size-4 text-muted" />
      ...
      <PcardLog query={query} onOpen={onOpen} />
    </section>
  );
}
  ...
  return (
    <section className="flex flex-col gap-3 pb-6">
      <button
        type="button"
        onClick={onBack}
        className="flex h-11 items-center gap-2 text-sm font-semibold text-navy"
      >
    ...
    <div className="flex flex-col gap-3">
      <article className="rounded-lg border border-line bg-navy p-3 text-cream">
        ...
        <p className="mt-1 text-sm leading-relaxed text-cream/90">
          Zone 31 is Riverside Unit. Confirm against the radio in your pocket — this is a
  ...
  return (
    <section className="flex flex-col gap-4 pb-6 print:p-0">
      <button
        type="button"
        onClick={onBack}
        className="flex h-11 items-center gap-2 text-sm font-semibold text-navy print:hidden"
      >
        <ArrowLeft className="size-4" />
        Forms
      </button>
